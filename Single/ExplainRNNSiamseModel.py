...
for i in range(len(subject_train)):
    ...
    train_loss_all = []
    val_loss_all = []
    train_dataset = SequentialFeaturesReader(path, include_subjects=subject_train[i], n_window=n_window, n_stride=n_stride, training=True)
    ...
    model = SiamseActionPerceptionModel(input_dim=len(features), inference=True, sequence_len=n_window)
    ...
    embds_all = torch.concatenate(embds_all, dim=0).view((-1, 32))
    ...
    for i_batch, (sample_batched, sample_batched_disp) in enumerate(zip(val_loader, val_loader_display)):
        ...
        preds = model.predict(X_test, n=10)
        y_test_all.append(y)
    ...
    #    shap_values = e.attribute(X_test_all[(i*100):(i+1) * 100], X_train_all)
