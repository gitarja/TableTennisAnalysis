...
def trainLASSO(X_train, y_train, alpha=0.1):
    model = LogisticRegression(penalty='l1', solver="liblinear", class_weight="balanced", fit_intercept=True,
                               max_iter=100000, C=alpha)
...
def evaluateModel(model, X_test, y_test, x_column):
    ...
    statistics_df = model.printSummary(x_column, print_df=False)

    return mcc, cm, acc, auc_score, f1, statistics_df
...
for features_group in ["important"]:

    all_acc, all_mcc = [], []
    # load subject cross validation
    mcc_list, cm_list, acc_list, auc_list, f1_list, statistic_list = [], [], [], [], [], []
    ...
    for a in [0.05]:
        ...
        for i in range(len(subject_train)):
            ...
            clf = trainLASSO(X_train, y_train, alpha=a)
            mcc, cm, acc, auc_score, f1, statistics_df = evaluateModel(clf, X_test, y_test, x_column)
